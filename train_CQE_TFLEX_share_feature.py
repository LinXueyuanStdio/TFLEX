...
class EntityProjection(nn.Module):
    ...
    def forward(self,
                q_feature, q_logic, q_time_logic, q_time_density,
                r_feature, r_logic, r_time_logic, r_time_density,
                t_feature, t_logic, t_time_logic, t_time_density):
        x = torch.cat([
        ...
        feature, logic, time_logic, time_density = torch.chunk(x, 4, dim=-1)
        ...
        return feature, logic, time_logic, time_density
...
class TimeProjection(nn.Module):
    ...
    def forward(self,
                q1_feature, q1_logic, q1_time_logic, q1_time_density,
                r_feature, r_logic, r_time_logic, r_time_density,
                q2_feature, q2_logic, q2_time_logic, q2_time_density):
        x = torch.cat([
...
class EntityIntersection(nn.Module):
    def __init__(self, dim):
        ...
        self.dim = dim
        self.feature_layer_1 = nn.Linear(self.dim * 4, self.dim)
    ...
    def forward(self, feature, logic, time_logic, time_density):
        # N x B x d
        logits = torch.cat([feature, logic, time_logic, time_density], dim=-1)  # N x B x 2d
...
class TemporalBefore(nn.Module):
    ...
    def forward(self, feature, logic, time_logic, time_density):
        ...
        theta_left = feature - time_logic / 2
        center = theta_left + L
        feature = center / 2
        time_logic = center

        logits = torch.cat([feature, time_logic, time_density], dim=-1)  # N x B x 3d
...
class TemporalAfter(nn.Module):
    ...
    def forward(self, feature, logic, time_logic, time_density):
        ...
        theta_right = feature + time_logic / 2
...
class FLEX(nn.Module):
    ...
    def build_parser(self):
        def And(q1, q2):
            q1_feature, q1_logic, q1_time_logic, q1_time_density = q1
            q2_feature, q2_logic, q2_time_logic, q2_time_density = q2
            ...
            return self.entity_intersection(feature, logic, time_logic, time_density)

        def And3(q1, q2, q3):
            q1_feature, q1_logic, q1_time_logic, q1_time_density = q1
            q2_feature, q2_logic, q2_time_logic, q2_time_density = q2
            q3_feature, q3_logic, q3_time_logic, q3_time_density = q3
        ...
        def Or(q1, q2):
            ...
            time_logic = torch.stack([q1_time_logic, q2_time_logic])
            time_density = torch.stack([q1_time_density, q2_time_density])
            return self.entity_union(feature, logic, time_logic, time_density)

        def Not(q):
            feature, logic, time_logic, time_density = q
            return self.entity_negation(feature, logic, time_logic, time_density)

        def TimeNot(q):
            feature, logic, time_logic, time_density = q
            return self.time_negation(feature, logic, time_logic, time_density)

        def EntityProjection2(e1, r1, t1):
            s_feature, s_logic, s_time_logic, s_time_density = e1
            r_feature, r_logic, r_time_logic, r_time_density = r1
            t_feature, t_logic, t_time_logic, t_time_density = t1
            return self.entity_projection(
                s_feature, s_logic, s_time_logic, s_time_density,
                r_feature, r_logic, r_time_logic, r_time_density,
                t_feature, t_logic, t_time_logic, t_time_density
            )
        ...
        def TimeProjection2(e1, r1, e2):
            s_feature, s_logic, s_time_logic, s_time_density = e1
            r_feature, r_logic, r_time_logic, r_time_density = r1
            o_feature, o_logic, o_time_logic, o_time_density = e2
            return self.time_projection(
                s_feature, s_logic, s_time_logic, s_time_density,
                r_feature, r_logic, r_time_logic, r_time_density,
                o_feature, o_logic, o_time_logic, o_time_density
            )
        ...
        def TimeAnd(q1, q2):
            ...
            time_logic = torch.stack([q1_time_logic, q2_time_logic])
            time_density = torch.stack([q1_time_density, q2_time_density])
            return self.time_intersection(feature, logic, time_logic, time_density)
        ...
        def TimeOr(q1, q2):
            ...
            time_logic = torch.stack([q1_time_logic, q2_time_logic])
            time_density = torch.stack([q1_time_density, q2_time_density])
            return self.time_union(feature, logic, time_logic, time_density)

        def TimeBefore(q):
            feature, logic, time_logic, time_density = q
            return self.time_before(feature, logic, time_logic, time_density)

        def TimeAfter(q):
            feature, logic, time_logic, time_density = q
            return self.time_after(feature, logic, time_logic, time_density)

        def TimeNext(q):
            feature, logic, time_logic, time_density = q
            return self.time_next(feature, logic, time_logic, time_density)
    ...
    def timestamp_feature(self, idx):
        return convert_to_feature(self.scale(self.timestamp_feature_embedding(idx)))
    ...
    def timestamp_token(self, idx) -> TYPE_token:
        feature = self.timestamp_feature(idx)
        logic = torch.zeros_like(feature).to(feature.device)
        time_logic = torch.zeros_like(feature).to(feature.device)
    ...
    def batch_predict(self, grouped_query: Dict[QueryStructure, torch.Tensor], grouped_idxs: Dict[QueryStructure, List[List[int]]]):
        ...
        def cat_to_tensor(token_list: List[TYPE_token]) -> TYPE_token:
            ...
            for x in token_list:
                feature.append(x[0])
                logic.append(x[1])
                time_logic.append(x[2])
                time_density.append(x[3])
    ...
    def scoring_timestamp(self, timestamp_feature, q: TYPE_token):
        feature, logic, time_logic, time_density = q
        distance = self.distance_between_timestamp_and_query(timestamp_feature, feature, time_logic, time_density)
        score = self.gamma - distance * self.modulus
        return score
