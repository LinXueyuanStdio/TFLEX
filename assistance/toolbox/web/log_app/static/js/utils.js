...
function convert_to_columns(column_order, column_dict, hidden_columns) {
    ...
    const max_depth = get_max_col_ord_depth(column_order);
    const columns = [];
...
function generate_columns(column_order, column_dict, hidden_columns, prefix, columns, depth, max_depth) {
    ...
    const keys = get_order_keys(column_order);
...
// 成功提示
const success_prompt = function (message, time) {
    prompt(message, 'alert-success', time);
};

// 警告提示
const warning_prompt = function (message, time) {
