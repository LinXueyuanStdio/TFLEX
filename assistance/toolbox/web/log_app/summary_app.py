...
from .server.utils import stringify_dict_key, colored_string
...
def summary_selections():
    ...
    try:
        if 'config_name' in request.json:
            logs = read_logs(all_data['remote_log_servers'], request.json['config_name'], all_data['root_log_dir'])
        elif 'log_names' in request.json:
            logs = read_logs(all_data['remote_log_servers'], request.json['log_names'], all_data['root_log_dir'], all_data['extra_data'])
        else:
...
def new_summary():
    ...
    try:
        ...
        summary_table = generate_summary_table(vertical, horizontals, method, criteria, results, result_maps, selected_data,
                                               all_data['remote_log_servers'], all_data['root_log_dir'], all_data['extra_data'], extra_summary)
...
def save_summary_api():
    ...
    try:
        ...
        print(colored_string("Save summary failed.", 'red'))
        print(e)
        import traceback
        traceback.print_exc()
...
def delete_summary_api():
    ...
    for summary_name in summary_names[::-1]:
        try:
            ...
            print(colored_string("Delete summary {} encountered an error.".format(summary_name), 'red'))
            print(repr(e))
