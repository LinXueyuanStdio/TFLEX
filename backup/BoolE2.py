...
class PercentagE(nn.Module):
    ...
    def forwardScoringAll(self,
        ...
        for query_structure in batch_queries_dict:
            ...
            all_axis.append(axis)
            all_arg.append(arg)
        all_axis = torch.cat(all_axis, dim=0)  # (B, 1, d)
        all_arg = torch.cat(all_arg, dim=0)  # (B, 1, d)
        # positive samples for non-union queries in this batch

        positive_axis = self.entity_embedding
        positive_axis = self.angle_scale(positive_axis, self.axis_scale)
        positive_axis = convert_to_axis(positive_axis)

        return torch.mm(all_axis, positive_axis.transpose(-1, -2)), all_idxs

    def loss(self, y_, y):
        return self.bce(y_, y)
