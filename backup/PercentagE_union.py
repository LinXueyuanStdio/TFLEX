...
class Union(nn.Module):
    ...
    def forward(self, axis, arg):
        # axis: 2 x N x B x d
        # arg:  2 x N x B x d
        return torch.max(axis, dim=0), torch.max(arg, dim=0)
        # logits = torch.cat([axis, arg], dim=-1)  # N x B x 2d
        #
        # axis_attention = F.softmax(self.axis_layer_2(F.relu(self.axis_layer_1(logits))), dim=0)
        # axis = torch.sum(axis_attention * axis, dim=0)
        #
        # arg = torch.sum(arg, dim=0) - torch.prod(arg, dim=0)
        # return axis, arg
...
class Negation(nn.Module):
    ...
    def neg_axis(self, axis):
        ...
        axis[indicator_positive] = axis[indicator_positive] - 1
        axis[indicator_negative] = axis[indicator_negative] + 1
        return axis
...
class PercentagE(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma,
        ...
        self.cone_union = Union(self.entity_dim, drop)
    ...
    def embed_query_cone(self, queries: torch.Tensor, query_structure: QueryStructure, idx: int):
        """
        迭代嵌入
        ...
        if all_relation_flag:
            ...
            for i in range(len(query_structure[-1])):
                ...
                idx += 1
        elif 'u' in self.query_name_dict[query_structure] and 'DNF' in self.query_name_dict[query_structure]:
            # 并运算
            print("union")
            axis_list = []
            arg_list = []
            for i in range(len(query_structure[0])):  # 把内部每个子结构都嵌入了，再执行 且运算
                q_axis, q_arg, idx = self.embed_query_cone(queries, query_structure[0][i], idx)
                axis_list.append(q_axis)
                arg_list.append(q_arg)

            stacked_axis = torch.stack(axis_list)
            stacked_arg = torch.stack(arg_list)

            q_axis, q_arg = self.cone_union(stacked_axis, stacked_arg)
        else:
            ...
            q_axis, q_arg = self.cone_intersection(stacked_axis, stacked_arg)
...
if __name__ == "__main__":
    ...
    model = PercentagE(nentity, nrelation, hidden_dim, 12, query_name_dict=query_name_dict, center_reg=0.02)
    ...
    subsampling_weight = torch.LongTensor([1, 1])
    batch_queries_dict = collections.defaultdict(torch.Tensor)
    batch_idxs_dict = collections.defaultdict(torch.Tensor)
    query_structure = ('e', ('r',))
    batch_queries_dict[query_structure] = torch.LongTensor([[1, 2]])
    batch_idxs_dict[query_structure] = [0, 0]
