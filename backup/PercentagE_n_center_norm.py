"""
@date: 2021/12/7
@description: 2个 axis [-inf,inf], 一个 arg [0,1]
"""
...
class Intersection(nn.Module):
    ...
    def forward(self, axis_1, axis_2, arg):
        # axis: N x B x d
        # arg:  N x B x d
        logits_1 = torch.cat([axis_1, arg], dim=-1)  # N x B x 2d
        axis_attention_1 = F.softmax(self.axis_1_layer_2(F.relu(self.axis_1_layer_1(logits_1))), dim=0)
        axis_1 = torch.sum(axis_attention_1 * axis_1, dim=0)

        logits_2 = torch.cat([axis_2, arg], dim=-1)  # N x B x 2d
        axis_attention_2 = F.softmax(self.axis_2_layer_2(F.relu(self.axis_2_layer_1(logits_2))), dim=0)
        axis_2 = torch.sum(axis_attention_2 * axis_2, dim=0)
...
class Negation(nn.Module):
    ...
    def forward(self, axis_1, axis_2, arg):
        logits_1 = torch.cat([axis_1, arg], dim=-1)  # B x 2d
        axis_1 = self.axis_1_layer_2(F.relu(self.axis_1_layer_1(logits_1)))

        logits_2 = torch.cat([axis_2, arg], dim=-1)  # B x 2d
        axis_2 = self.axis_2_layer_2(F.relu(self.axis_2_layer_1(logits_2)))
