"""
@date: 2021/12/7
@description: p: tucker i: transformer
"""
...
class Projection(nn.Module):
    def __init__(self, dim, hidden_dropout1=0.1, hidden_dropout2=0.1):
        super(Projection, self).__init__()
        self.entity_dim = 2 * dim
        self.relation_dim = 2 * dim
        self.W = nn.Parameter(torch.FloatTensor(np.random.uniform(-0.01, 0.01, (self.entity_dim, self.relation_dim, self.entity_dim))))
    ...
    def forward(self, q_axis, q_arg, r_axis, r_arg):
        h = torch.cat([q_axis, q_arg], dim=-1)
        r = torch.cat([r_axis, r_arg], dim=-1)
        h = self.bn0(h.view(-1, self.entity_dim)).view(-1, 1, self.entity_dim)
        ...
        axis, arg = torch.chunk(t, 2, dim=-1)
...
class Intersection(nn.Module):
    ...
    def forward(self, axis, arg):
        ...
        # 1. calculate axis
        # 1.1 attention
        logits_1 = torch.cat([axis - arg, axis + arg], dim=-1)  # N x B x 2d
        axis_attention_1 = F.softmax(self.axis_1_layer_2(F.relu(self.axis_1_layer_1(logits_1))), dim=0)
        axis = self.calculate_axis(axis, axis_attention_1)
...
class TTE(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma,
        ...
        self.cone_proj = Projection(self.entity_dim)
...
if __name__ == "__main__":
    ...
    model = TTE(nentity, nrelation, hidden_dim, 12, query_name_dict=query_name_dict, center_reg=0.02)
